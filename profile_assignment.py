...
def fill_all_municipalities():
    ...
    not_included = []
    keys =folders.keys()
    average_com=com_profile.mean(axis=1)
    average_res = res_profile.mean(axis=1)
    for key in keys:
        if key in cols:
            continue
        else:
    ...
    res_profile.to_csv('Demand_calculator/residential_profiles.csv',index=False)
    com_profile.to_csv('Demand_calculator/commercial_profiles.csv',index=False)
...
def get_daily_commercial_profile():
    com_profile = pd.read_csv('Demand_calculator/commercial_profiles.csv')
