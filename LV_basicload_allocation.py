...
class load_allocation:
    ...
    def import_demand(self):
        el_dmd_name = 'Square_zones_dmd/'+self.grid_dict[self.grids_name]+'/'+self.grids_name+'_el_dmd.geojson'
    ...
    def voronoi(self, save_plot=False):
        ...
        if not os.path.exists('results'):
            os.makedirs('results')
        if save_plot:
            plt.savefig('results/'+self.grids_name+'_voronoi.png', dpi=300, bbox_inches='tight')
    ...
    def transform_from_pkl(self):
        path = 'Demand_calculator/Municipality_normalized_profiles.pkl'
        ...
        for key in keys:
            ...
            try:
                ...
                print(key)
                continue
        commercial.to_csv('Demand_calculator/commercial_profiles.csv', index=False)
        residential.to_csv('Demand_calculator/residential_profiles.csv', index=False)
